# Go SDK - Issue Statistics

**Generated:** 2026-01-02
**Repository:** temporalio/sdk-go
**Data Source:** [issues.md](../repos/temporalio-sdk-go/issues.md)

## Related Documents

← [Summary](summary.md) · [Analysis](go.md) · [Contributors](contributors.md) · [Recent](stats-recent.md) · [All Stats](stats-all.md)

**Other Statistics:** [API](stats-temporalio-api.md) · [Features](stats-features.md) · [.NET](stats-dotnet.md) · [Java](stats-java.md) · [PHP](stats-php.md) · [Python](stats-python.md) · [Ruby](stats-ruby.md) · [TypeScript](stats-typescript.md) · [Server](stats-server.md)

---

## Summary

| Metric | Value |
|--------|-------|
| Total Open Issues | 163 |
| Bugs | 59 |
| Enhancements/Features | 89 |
| Security Issues | 1 |
| Total Upvotes (👍) | 112 |
| Total Comments | 214 |
| Issues with Upvotes | 41 (25%) |
| Issues with Comments | 86 (53%) |
| Average Age (days) | 977 |
| Stale Issues (>12 months) | 101 (62%) |

---

## Age Distribution

| Age | Count | % |
|-----|-------|---|
| Last 30 days | 1 | 1% |
| 1-3 months | 10 | 6% |
| 3-6 months | 7 | 4% |
| 6-12 months | 16 | 10% |
| 1-2 years | 31 | 19% |
| 2-3 years | 29 | 18% |
| 3+ years | 69 | 42% |

---

## Top Issues by Upvotes

| Rank | Issue | 👍 | 💬 | Title |
|------|-------|-----|-----|-------|
| 1 | [#916](../repos/temporalio-sdk-go/issues.md#916) | 21 | 0 | Ability to mock/intercept side effect calls in test suite |
| 2 | [#181](../repos/temporalio-sdk-go/issues.md#181) | 16 | 0 | Add ability for a clean worker shutdown that waits for a ses... |
| 3 | [#89](../repos/temporalio-sdk-go/issues.md#89) | 7 | 1 | Refactor SDK to avoid type aliasing |
| 4 | [#829](../repos/temporalio-sdk-go/issues.md#829) | 6 | 5 | Use interceptor logger for all logs related to a workflow or... |
| 5 | [#1282](../repos/temporalio-sdk-go/issues.md#1282) | 5 | 5 | Heartbeat timeout not raised while testing |
| 6 | [#1352](../repos/temporalio-sdk-go/issues.md#1352) | 4 | 6 | Serialization context for codecs and converters |
| 7 | [#1149](../repos/temporalio-sdk-go/issues.md#1149) | 4 | 3 | Allow custom trace attribute keys |
| 8 | [#866](../repos/temporalio-sdk-go/issues.md#866) | 4 | 1 | TestWorkflowEnvironment doesn't enforce WorkflowExecutionTim... |
| 9 | [#313](../repos/temporalio-sdk-go/issues.md#313) | 4 | 2 | Allow including local activity input into the history |
| 10 | [#2109](../repos/temporalio-sdk-go/issues.md#2109) | 2 | 1 | MutableSideEffect in test framework ignores cmp function |
| 11 | [#1582](../repos/temporalio-sdk-go/issues.md#1582) | 2 | 0 | Updated context functions to match context library |
| 12 | [#1410](../repos/temporalio-sdk-go/issues.md#1410) | 2 | 0 | Consider migrating away from golang/mock |
| 13 | [#1208](../repos/temporalio-sdk-go/issues.md#1208) | 2 | 2 | Canceling a context with multiple child contexts can be non ... |
| 14 | [#884](../repos/temporalio-sdk-go/issues.md#884) | 2 | 3 | [Feature Request] Add ability to support passing sessions fr... |
| 15 | [#718](../repos/temporalio-sdk-go/issues.md#718) | 2 | 1 | Add metric for time passed from the activity scheduling to t... |

---

## Top Issues by Priority Score

Priority = Upvotes × 2 + Comments

| Rank | Issue | Score | 👍 | 💬 | Title |
|------|-------|-------|-----|-----|-------|
| 1 | [#916](../repos/temporalio-sdk-go/issues.md#916) | 42 | 21 | 0 | Ability to mock/intercept side effect calls in tes... |
| 2 | [#181](../repos/temporalio-sdk-go/issues.md#181) | 32 | 16 | 0 | Add ability for a clean worker shutdown that waits... |
| 3 | [#829](../repos/temporalio-sdk-go/issues.md#829) | 17 | 6 | 5 | Use interceptor logger for all logs related to a w... |
| 4 | [#1282](../repos/temporalio-sdk-go/issues.md#1282) | 15 | 5 | 5 | Heartbeat timeout not raised while testing |
| 5 | [#89](../repos/temporalio-sdk-go/issues.md#89) | 15 | 7 | 1 | Refactor SDK to avoid type aliasing |
| 6 | [#1352](../repos/temporalio-sdk-go/issues.md#1352) | 14 | 4 | 6 | Serialization context for codecs and converters |
| 7 | [#1149](../repos/temporalio-sdk-go/issues.md#1149) | 11 | 4 | 3 | Allow custom trace attribute keys |
| 8 | [#313](../repos/temporalio-sdk-go/issues.md#313) | 10 | 4 | 2 | Allow including local activity input into the hist... |
| 9 | [#866](../repos/temporalio-sdk-go/issues.md#866) | 9 | 4 | 1 | TestWorkflowEnvironment doesn't enforce WorkflowEx... |
| 10 | [#642](../repos/temporalio-sdk-go/issues.md#642) | 9 | 1 | 7 | Inconsistent behavior when asserting invocations |
| 11 | [#475](../repos/temporalio-sdk-go/issues.md#475) | 9 | 0 | 9 | Panic when querying during Workflow failure + retr... |
| 12 | [#1330](../repos/temporalio-sdk-go/issues.md#1330) | 8 | 0 | 8 | Add TaskQueue to TracerStartSpanOptions |
| 13 | [#1114](../repos/temporalio-sdk-go/issues.md#1114) | 8 | 1 | 6 | Support activity functions with variadic arguments... |
| 14 | [#986](../repos/temporalio-sdk-go/issues.md#986) | 7 | 1 | 5 | AwaitWithTimeout does not cancel its timer when co... |
| 15 | [#937](../repos/temporalio-sdk-go/issues.md#937) | 7 | 0 | 7 | Add ability to keep session open in case of a work... |

---

## Top Issues by Comments

| Rank | Issue | 💬 | 👍 | Title |
|------|-------|-----|-----|-------|
| 1 | [#475](../repos/temporalio-sdk-go/issues.md#475) | 9 | 0 | Panic when querying during Workflow failure + retry  |
| 2 | [#1330](../repos/temporalio-sdk-go/issues.md#1330) | 8 | 0 | Add TaskQueue to TracerStartSpanOptions |
| 3 | [#937](../repos/temporalio-sdk-go/issues.md#937) | 7 | 0 | Add ability to keep session open in case of a worker restart... |
| 4 | [#642](../repos/temporalio-sdk-go/issues.md#642) | 7 | 1 | Inconsistent behavior when asserting invocations |
| 5 | [#1352](../repos/temporalio-sdk-go/issues.md#1352) | 6 | 4 | Serialization context for codecs and converters |
| 6 | [#1114](../repos/temporalio-sdk-go/issues.md#1114) | 6 | 1 | Support activity functions with variadic arguments   |
| 7 | [#1282](../repos/temporalio-sdk-go/issues.md#1282) | 5 | 5 | Heartbeat timeout not raised while testing |
| 8 | [#986](../repos/temporalio-sdk-go/issues.md#986) | 5 | 1 | AwaitWithTimeout does not cancel its timer when condition is... |
| 9 | [#829](../repos/temporalio-sdk-go/issues.md#829) | 5 | 6 | Use interceptor logger for all logs related to a workflow or... |
| 10 | [#659](../repos/temporalio-sdk-go/issues.md#659) | 5 | 0 | Possibility to deprecate attribute `Control` |

---

## Recent Issues (Last 30 Days)

| Date | Issue | 👍 | Title |
|------|-------|-----|-------|
| 2025-12-08 | [#2131](../repos/temporalio-sdk-go/issues.md#2131) | 0 | Configurable minRPCTimeout for activity worker instead of us... |

---

## Label Distribution

| Label | Count |
|-------|-------|
| enhancement | 89 |
| potential-bug | 45 |
| bug | 15 |
| external dependency | 5 |
| mend: dependency security vulnerability | 1 |
| next-gen | 1 |

---

## Bugs (59 issues)

| Issue | 👍 | 💬 | Title |
|-------|-----|-----|-------|
| [#1282](../repos/temporalio-sdk-go/issues.md#1282) | 5 | 5 | Heartbeat timeout not raised while testing |
| [#866](../repos/temporalio-sdk-go/issues.md#866) | 4 | 1 | TestWorkflowEnvironment doesn't enforce WorkflowExecutionTim... |
| [#2109](../repos/temporalio-sdk-go/issues.md#2109) | 2 | 1 | MutableSideEffect in test framework ignores cmp function |
| [#1208](../repos/temporalio-sdk-go/issues.md#1208) | 2 | 2 | Canceling a context with multiple child contexts can be non ... |
| [#351](../repos/temporalio-sdk-go/issues.md#351) | 2 | 0 | [Testing] Cancellation of Parent Workflow with Child Workflo... |
| [#167](../repos/temporalio-sdk-go/issues.md#167) | 2 | 3 | Activity times out in test environment with child workflow |
| [#2107](../repos/temporalio-sdk-go/issues.md#2107) | 1 | 0 | Panic in temporal unit test framework. |
| [#1906](../repos/temporalio-sdk-go/issues.md#1906) | 1 | 1 | workflowcheck crash (index out of range) |
| [#1100](../repos/temporalio-sdk-go/issues.md#1100) | 1 | 2 | TestWorkflowEnvironment should surface errors and respect fa... |
| [#982](../repos/temporalio-sdk-go/issues.md#982) | 1 | 4 | Cannot register mock activity with string name on test workf... |
| [#642](../repos/temporalio-sdk-go/issues.md#642) | 1 | 7 | Inconsistent behavior when asserting invocations |
| [#2103](../repos/temporalio-sdk-go/issues.md#2103) | 0 | 3 | Cosmetic bug in logger |
| [#2090](../repos/temporalio-sdk-go/issues.md#2090) | 0 | 0 | Starting a child workflow in tests leaks goroutines |
| [#2066](../repos/temporalio-sdk-go/issues.md#2066) | 0 | 3 | Lost messages from workflow.NewChannel(ctx) |
| [#2039](../repos/temporalio-sdk-go/issues.md#2039) | 0 | 1 | Joining errors causes TestWorkflowEnvironment to hide panics... |
| [#2025](../repos/temporalio-sdk-go/issues.md#2025) | 0 | 0 | Fix DescribeTaskQueueEnhanced w/ stats to use non-deprecated... |
| [#2005](../repos/temporalio-sdk-go/issues.md#2005) | 0 | 0 | OnWorkflow test mocker doesn't propagate context headers |
| [#1961](../repos/temporalio-sdk-go/issues.md#1961) | 0 | 3 | Getting a strange error when writing tests which does not oc... |
| [#1929](../repos/temporalio-sdk-go/issues.md#1929) | 0 | 2 | OTel emitted metrics do not match behavior described in the ... |
| [#1869](../repos/temporalio-sdk-go/issues.md#1869) | 0 | 0 | SetOnChildWorkflowCompletedListener doesn't work as expected |
| [#1846](../repos/temporalio-sdk-go/issues.md#1846) | 0 | 0 | AsTime() on a nil pointer converts to  Jan. 1st, 1970 at mid... |
| [#1838](../repos/temporalio-sdk-go/issues.md#1838) | 0 | 0 | Non-deterministic workflow code can send incorrect WFT compl... |
| [#1668](../repos/temporalio-sdk-go/issues.md#1668) | 0 | 0 | When deserializing typed search attributes, invalid values s... |
| [#1650](../repos/temporalio-sdk-go/issues.md#1650) | 0 | 0 | List of flaky tests |
| [#1624](../repos/temporalio-sdk-go/issues.md#1624) | 0 | 1 | Signal sent to Selector can be lost if Default path blocks  |
| [#1574](../repos/temporalio-sdk-go/issues.md#1574) | 0 | 1 | Batched heartbeat got canceled due to caller context cancell... |
| [#1536](../repos/temporalio-sdk-go/issues.md#1536) | 0 | 0 | TERMINATE_IF_RUNNING doesn't work for duplicate child workfl... |
| [#1496](../repos/temporalio-sdk-go/issues.md#1496) | 0 | 0 | Workflow outcome is inconsistent between test env and real s... |
| [#1468](../repos/temporalio-sdk-go/issues.md#1468) | 0 | 2 | Interceptors are skipped for mocked activities |
| [#1427](../repos/temporalio-sdk-go/issues.md#1427) | 0 | 0 | TestWorkflowEnvironment workflow Context does not block when... |

*...and 29 more bugs*

---

## Stale Issues (101 issues with no activity >12 months)

Top stale issues by upvotes:

| Issue | 👍 | Last Updated | Title |
|-------|-----|--------------|-------|
| [#916](../repos/temporalio-sdk-go/issues.md#916) | 21 | 2022-09-22 | Ability to mock/intercept side effect calls in tes... |
| [#181](../repos/temporalio-sdk-go/issues.md#181) | 16 | 2020-07-07 | Add ability for a clean worker shutdown that waits... |
| [#829](../repos/temporalio-sdk-go/issues.md#829) | 6 | 2023-02-02 | Use interceptor logger for all logs related to a w... |
| [#1282](../repos/temporalio-sdk-go/issues.md#1282) | 5 | 2023-11-06 | Heartbeat timeout not raised while testing |
| [#1582](../repos/temporalio-sdk-go/issues.md#1582) | 2 | 2024-08-08 | Updated context functions to match context library |
| [#1410](../repos/temporalio-sdk-go/issues.md#1410) | 2 | 2024-03-04 | Consider migrating away from golang/mock |
| [#1208](../repos/temporalio-sdk-go/issues.md#1208) | 2 | 2024-03-04 | Canceling a context with multiple child contexts c... |
| [#884](../repos/temporalio-sdk-go/issues.md#884) | 2 | 2024-12-16 | [Feature Request] Add ability to support passing s... |
| [#718](../repos/temporalio-sdk-go/issues.md#718) | 2 | 2022-04-07 | Add metric for time passed from the activity sched... |
| [#351](../repos/temporalio-sdk-go/issues.md#351) | 2 | 2021-01-26 | [Testing] Cancellation of Parent Workflow with Chi... |
| [#167](../repos/temporalio-sdk-go/issues.md#167) | 2 | 2024-03-15 | Activity times out in test environment with child ... |
| [#61](../repos/temporalio-sdk-go/issues.md#61) | 2 | 2024-12-17 | Remove mocks package |
| [#1646](../repos/temporalio-sdk-go/issues.md#1646) | 1 | 2024-09-23 | SetStartTime for activity test env |
| [#1351](../repos/temporalio-sdk-go/issues.md#1351) | 1 | 2024-01-16 | Support interceptor on WorkflowRun.Get |
| [#1100](../repos/temporalio-sdk-go/issues.md#1100) | 1 | 2023-05-08 | TestWorkflowEnvironment should surface errors and ... |
| [#949](../repos/temporalio-sdk-go/issues.md#949) | 1 | 2022-12-02 | Allow user to override default maximum activity at... |
| [#754](../repos/temporalio-sdk-go/issues.md#754) | 1 | 2024-12-17 | gRPC Proxy Integration Test |
| [#642](../repos/temporalio-sdk-go/issues.md#642) | 1 | 2024-03-15 | Inconsistent behavior when asserting invocations |
| [#497](../repos/temporalio-sdk-go/issues.md#497) | 1 | 2021-10-28 | [Test suite] Query after signal works in SDK, but ... |
| [#458](../repos/temporalio-sdk-go/issues.md#458) | 1 | 2021-11-16 | TestWorkflowEnvironment: Fail RegisterDelayedCallb... |

*...and 81 more stale issues*
